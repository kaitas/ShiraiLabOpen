...
namespace graphicsNS
{
    ...
    const COLOR_ARGB FILTER  = D3DCOLOR_ARGB(  0,  0,  0,  0);  // use to specify drawing with colorFilter	// colorFilterで描画することを指定するために使用
    const COLOR_ARGB ALPHA25 = D3DCOLOR_ARGB( 64,255,255,255);  // AND with color to get 25% alpha			// 色とANDを実行して、25%アルファを取得
    const COLOR_ARGB ALPHA50 = D3DCOLOR_ARGB(128,255,255,255);  // AND with color to get 50% alpha			// 色とANDを実行して、50%アルファを取得
    const COLOR_ARGB BACK_COLOR = NAVY;                         // background color of game					//ゲームの背景色

    enum DISPLAY_MODE{TOGGLE, FULLSCREEN, WINDOW};
}

// SpriteData: The properties required by Graphics::drawSprite to draw a sprite
// SpriteData: スプライトを描画するGraphics::drawSpriteが必要とするプロパティ  P141
struct SpriteData
{
    int         width;      // width of sprite in pixels	// スプライトの幅(ピクセル単位)
    int         height;     // height of sprite in pixels	// スプライトの高さ(ピクセル単位)
    float       x;          // screen location (top left corner of sprite)// 画面位置(スプライトの左上隅)
    float       y;			// 
    float       scale;      // <1 smaller, >1 bigger		// <1は縮小、 >1は拡大
    float       angle;      // rotation angle in radians	// 回転角度(ラジアン単位)
    RECT        rect;       // used to select an image from a larger texture //大きなテクスチャから1つの画像を選択するときに使用
    LP_TEXTURE  texture;    // pointer to texture			//テクスチャへのポインタ
    bool        flipHorizontal; // true to flip sprite horizontally (mirror)//スプライトを水平方向に反転させる場合、true
    bool        flipVertical;   // true to flip sprite vertically			//スプライトを水平方向に反転させる場合、true
};
