...
// add by Akihiko SHIRAI, for KAIT Game Programming 2015
...
void Spacewar::initialize(HWND hwnd)
{
	...
	//âFíàëDâΩë‰ï`ÇØÇÈÇ©íßêÌ
	for (int i=0; i<MAX_SHIPS; i++ ) {
		if (!ships[i].initialize(graphics,SHIP_WIDTH,
			SHIP_HEIGHT, SHIP_COLS, &shipTexture)) {
			char errText[255];
			sprintf(errText, "Error initializing ships[%d]", i );
			throw(GameError(gameErrorNS::FATAL_ERROR, errText ));
		}
//		ships[i].setX(i)+rand(); ships[i].setY(i); //àÍíºê¸ÇæÇ∆Ç¬Ç‹ÇÁÇ»Ç¢ÇÃÇ≈ÉâÉìÉ_ÉÄÇ…ÇµÇ‹ÇµÇΩ
